/**
 * HIP port of GPU Temporal Data Mining.
 **/
...
extern "C" hipError_t tdm_launch_count_candidates_hip(
	ubyte targetEvent,
	float minInterval,
	float maxInterval,
	uint2* startRecords,
...
extern "C" hipError_t tdm_launch_write_candidates_hip(
	ubyte targetEvent,
	float minInterval,
	float maxInterval,
	uint2* startRecords,
...
void setupGpu(hipStream_t stream)
{
	record_region_start("device_side_buffer_setup");

	hip_check(hipMalloc((void**) &d_events, eventSize * sizeof(ubyte)), "failed to allocate d_events");
	hip_check(hipMalloc((void**) &d_times, eventSize * sizeof(float)), "failed to allocate d_times");
	hip_check(hipMalloc((void**) &d_episodeCandidates, maxCandidates * sizeof(ubyte)), "failed to allocate d_episodeCandidates");
	hip_check(hipMalloc((void**) &d_episodeIntervals, maxIntervals * sizeof(float)), "failed to allocate d_episodeIntervals");
	hip_check(hipMalloc((void**) &d_episodeSupport, maxCandidates * sizeof(uint)), "failed to allocate d_episodeSupport");
	hip_check(hipMalloc((void**) &d_startRecords, MaxRecords * sizeof(uint2)), "failed to allocate d_startRecords");
	hip_check(hipMalloc((void**) &d_foundRecords, MaxRecords * sizeof(uint2)), "failed to allocate d_foundRecords");
	...
	hip_check(hipMalloc((void**) &d_recCount, MaxRecords * sizeof(uint)), "failed to allocate d_recCount");
	hip_check(hipMalloc((void**) &d_recOffSet, MaxRecords * sizeof(uint)), "failed to allocate d_recOffSet");

	record_region_end(0);

	record_region_start("device_side_h2d_copy");

	hip_check(hipMemcpyAsync(d_events, h_events, eventSize, hipMemcpyHostToDevice, stream), "failed to copy d_events");
	hip_check(hipMemcpyAsync(d_times, h_times, eventSize * sizeof(float), hipMemcpyHostToDevice, stream), "failed to copy d_times");
	...
	hip_check(hipStreamSynchronize(stream), "failed to synchronize setup H2D copies");
...
int main(int argc, char** argv)
{
	...
	if (app_argc != 7) {
		printf("Usage: tdm_hip <data path> <intervals path> <episodes path> <threads> <max records> <max candidates>\n");
	...
	do {
		...
		for (unsigned int candidate_idx = 0; candidate_idx < numCandidates; candidate_idx++) {
			...
			hip_check(hipStreamSynchronize(stream), "failed to synchronize d_startRecords copy");
			...
			for (int level = 2; level <= maxLevel; level++) {
				...
				hip_check(
					tdm_launch_count_candidates_hip(
				...
				hip_check(hipStreamSynchronize(stream), "failed to synchronize countCandidates kernel");
				...
				hip_check(hipStreamSynchronize(stream), "failed to synchronize d_recCount copy");
				...
				hip_check(hipStreamSynchronize(stream), "failed to synchronize d_recOffSet copy");
				...
				hip_check(
					tdm_launch_write_candidates_hip(
						currentEpisode[level_idx],
						temporalConstraint[0],
						temporalConstraint[1],
						d_startRecords,
				...
				hip_check(hipStreamSynchronize(stream), "failed to synchronize writeCandidates kernel");
			...
			hip_check(hipStreamSynchronize(stream), "failed to synchronize d_startRecords read");
	...
	hipFree(d_events);
	hipFree(d_times);
	hipFree(d_episodeCandidates);
	hipFree(d_episodeIntervals);
	hipFree(d_episodeSupport);
	hipFree(d_startRecords);
	hipFree(d_foundRecords);
	hipFree(d_recCount);
	hipFree(d_recOffSet);
