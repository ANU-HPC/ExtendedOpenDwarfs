...
int main(int argc, char** argv)
{
    ...
    for (k = 0; k < num_matrices; k++) {
        HIP_CHECK(hipMalloc((void**)&csr_ap_d[k], sizeof(unsigned int) * (csr[k].num_rows + 1)));
        HIP_CHECK(hipMalloc((void**)&csr_aj_d[k], sizeof(unsigned int) * csr[k].num_nonzeros));
        HIP_CHECK(hipMalloc((void**)&csr_ax_d[k], sizeof(float) * csr[k].num_nonzeros));
        HIP_CHECK(hipMalloc((void**)&x_d[k], sizeof(float) * csr[k].num_cols));
        HIP_CHECK(hipMalloc((void**)&y_d[k], sizeof(float) * csr[k].num_rows));
    }
    ...
    for (i = 0; i < num_execs; i++) {
        ...
        for (k = 0; k < num_matrices; k++) {
            ...
            do {
                ...
                HIP_CHECK(hipMemcpy(csr_ap_d[k], csr[k].Ap,
                                      sizeof(unsigned int) * (csr[k].num_rows + 1),
                                      hipMemcpyHostToDevice));
                HIP_CHECK(hipMemcpy(csr_aj_d[k], csr[k].Aj,
                                      sizeof(unsigned int) * csr[k].num_nonzeros,
                                      hipMemcpyHostToDevice));
                HIP_CHECK(hipMemcpy(csr_ax_d[k], csr[k].Ax,
                                      sizeof(float) * csr[k].num_nonzeros,
                                      hipMemcpyHostToDevice));
                HIP_CHECK(hipMemcpy(x_d[k], x_host,
                                      sizeof(float) * csr[k].num_cols,
                                      hipMemcpyHostToDevice));
                HIP_CHECK(hipMemcpy(y_d[k], y_host,
                ...
                HIP_CHECK(hipMemcpy(device_out[k], y_d[k],
                                      sizeof(float) * csr[k].num_rows,
    ...
    for (k = 0; k < num_matrices; k++) {
        HIP_CHECK(hipFree(csr_ap_d[k]));
        HIP_CHECK(hipFree(csr_aj_d[k]));
        HIP_CHECK(hipFree(csr_ax_d[k]));
        HIP_CHECK(hipFree(x_d[k]));
        HIP_CHECK(hipFree(y_d[k]));
    }
