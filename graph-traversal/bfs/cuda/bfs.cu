...
int main(int argc, char** argv)
{
	...
	cuda_check(cudaMalloc((void**) &d_graph_nodes, sizeof(Node) * no_of_nodes), "failed to allocate d_graph_nodes");
	cuda_check(cudaMalloc((void**) &d_graph_edges, sizeof(int) * edge_list_size), "failed to allocate d_graph_edges");
	cuda_check(cudaMalloc((void**) &d_graph_mask, sizeof(int) * no_of_nodes), "failed to allocate d_graph_mask");
	cuda_check(cudaMalloc((void**) &d_updating_graph_mask, sizeof(int) * no_of_nodes), "failed to allocate d_updating_graph_mask");
	cuda_check(cudaMalloc((void**) &d_graph_visited, sizeof(int) * no_of_nodes), "failed to allocate d_graph_visited");
	cuda_check(cudaMalloc((void**) &d_cost, sizeof(int) * no_of_nodes), "failed to allocate d_cost");
	cuda_check(cudaMalloc((void**) &d_over, sizeof(int)), "failed to allocate d_over");
	...
	do {
		...
		cuda_check(cudaMemcpyAsync(d_graph_nodes, h_graph_nodes, sizeof(Node) * no_of_nodes, cudaMemcpyHostToDevice, stream), "failed to copy d_graph_nodes");
		cuda_check(cudaMemcpyAsync(d_graph_edges, h_graph_edges, sizeof(int) * edge_list_size, cudaMemcpyHostToDevice, stream), "failed to copy d_graph_edges");
		cuda_check(cudaMemcpyAsync(d_graph_mask, h_graph_mask, sizeof(int) * no_of_nodes, cudaMemcpyHostToDevice, stream), "failed to copy d_graph_mask");
		cuda_check(cudaMemcpyAsync(d_updating_graph_mask, h_updating_graph_mask, sizeof(int) * no_of_nodes, cudaMemcpyHostToDevice, stream), "failed to copy d_updating_graph_mask");
		cuda_check(cudaMemcpyAsync(d_graph_visited, h_graph_visited, sizeof(int) * no_of_nodes, cudaMemcpyHostToDevice, stream), "failed to copy d_graph_visited");
		cuda_check(cudaMemcpyAsync(d_cost, h_cost, sizeof(int) * no_of_nodes, cudaMemcpyHostToDevice, stream), "failed to copy d_cost");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize initial H2D copies");
		...
		do {
			...
			cuda_check(cudaMemcpyAsync(d_over, &stop, sizeof(int), cudaMemcpyHostToDevice, stream), "failed to copy stop flag");
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize stop flag copy");
			...
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize kernel1");
			...
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize kernel2");

			record_region_end(k);

			record_region_start("device_side_d2h_copy");

			cuda_check(cudaMemcpyAsync(&stop, d_over, sizeof(int), cudaMemcpyDeviceToHost, stream), "failed to read stop flag");
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize stop flag read");
	...
	cuda_check(cudaMemcpyAsync(h_cost, d_cost, sizeof(int) * no_of_nodes, cudaMemcpyDeviceToHost, stream), "failed to read d_cost");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize d_cost read");
