...
int main(int argc, char** argv)
{
    ...
    CUDA_CHECK(cudaMalloc((void**)&J_cuda, sizeof(float) * (size_I + 1)));
    CUDA_CHECK(cudaMalloc((void**)&C_cuda, sizeof(float) * size_I));
    CUDA_CHECK(cudaMalloc((void**)&E_C, sizeof(float) * size_I));
    CUDA_CHECK(cudaMalloc((void**)&W_C, sizeof(float) * size_I));
    CUDA_CHECK(cudaMalloc((void**)&N_C, sizeof(float) * size_I));
    CUDA_CHECK(cudaMalloc((void**)&S_C, sizeof(float) * size_I));
    ...
    for (int iter = 0; iter < niter; iter++) {
        ...
        do {
            ...
            CUDA_CHECK(cudaMemcpy(O, J_cuda, sizeof(float) * size_I, cudaMemcpyDeviceToHost));
