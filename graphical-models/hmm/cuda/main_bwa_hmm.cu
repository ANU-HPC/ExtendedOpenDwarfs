...
static float dot_production(int n, float* paramA, int offsetA, float* paramB, int offsetB, cudaStream_t stream)
{
	if (n <= 0) {
		return 0.0f;
	}
	...
	cuda_check(cudaMalloc((void**) &partialSum_d, n * sizeof(float)), "failed to allocate partialSum_d");

	record_region_end(0);

	record_region_start("device_side_h2d_copy");

	cuda_check(cudaMemcpyAsync(partialSum_d, partialSum, n * sizeof(float), cudaMemcpyHostToDevice, stream), "failed to copy partialSum_d");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize partialSum_d copy");
	...
	s_dot_kernel_naive<<<blocks, threads, 0, stream>>>(n, paramA, offsetA, paramB, offsetB, partialSum_d);
	cuda_check(cudaGetLastError(), "failed to launch s_dot_kernel_naive");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize s_dot_kernel_naive");

	record_region_end(0);

	record_region_start("device_side_d2h_copy");

	cuda_check(cudaMemcpyAsync(partialSum, partialSum_d, n * sizeof(float), cudaMemcpyDeviceToHost, stream), "failed to copy partialSum");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize partialSum read");
...
static void mat_vec_mul(char trans, int m, int n, float* A, int lda, float* x, int offsetX, float* y, int offsetY, cudaStream_t stream)
{
	...
	if (trans == 't' || trans == 'T') {
		mvm_trans_kernel_naive<<<globalWorkSize / localWorkSize, localWorkSize, 0, stream>>>(
			m, n, A, lda, x, offsetX, y, offsetY);
		cuda_check(cudaGetLastError(), "failed to launch mvm_trans_kernel_naive");
	} else {
		mvm_non_kernel_naive<<<globalWorkSize / localWorkSize, localWorkSize, 0, stream>>>(
			m, n, A, lda, x, offsetX, y, offsetY);
		cuda_check(cudaGetLastError(), "failed to launch mvm_non_kernel_naive");
	}

	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize matvec kernel");

	record_region_end(0);
}

static float calc_alpha(cudaStream_t stream)
{
	...
	init_alpha_dev<<<nblocks, threads_per_block, 0, stream>>>(b_d, pi_d, nstates, alpha_d, ones_n_d, obs[0]);
	cuda_check(cudaGetLastError(), "failed to launch init_alpha_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize init_alpha_dev");
	...
	scale_alpha_dev<<<nblocks, threads_per_block, 0, stream>>>(nstates, alpha_d, tmp, scale_factors[0]);
	cuda_check(cudaGetLastError(), "failed to launch scale_alpha_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize scale_alpha_dev");
	...
	for (int t = 1; t < length; t++) {
		...
		cuda_check(cudaGetLastError(), "failed to launch calc_alpha_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize calc_alpha_dev");
		...
		scale_alpha_dev<<<nblocks, threads_per_block, 0, stream>>>(nstates, alpha_d, offset_cur, scale_factors[t]);
		cuda_check(cudaGetLastError(), "failed to launch scale_alpha_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize scale_alpha_dev");
...
static int calc_beta(cudaStream_t stream)
{
	int threads_per_block = MAX_THREADS_PER_BLOCK;
	int nblocks = (nstates + threads_per_block - 1) / threads_per_block;
	...
	init_beta_dev<<<nblocks, threads_per_block, 0, stream>>>(nstates, beta_d, offset, scale_factors[length - 1]);
	cuda_check(cudaGetLastError(), "failed to launch init_beta_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize init_beta_dev");
	...
	for (int t = length - 2; t >= 0; t--) {
		...
		calc_beta_dev<<<nblocks, threads_per_block, 0, stream>>>(beta_d, b_d, scale_factors[t], nstates, obs[t + 1], t);
		cuda_check(cudaGetLastError(), "failed to launch calc_beta_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize calc_beta_dev");
		...
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize beta scratch copy");
		record_region_end(t);
	}
...
static void calc_gamma_sum(cudaStream_t stream)
{
	int threads_per_block = MAX_THREADS_PER_BLOCK;
	int nblocks = (nstates + threads_per_block - 1) / threads_per_block;
	...
	cuda_check(cudaMemcpyAsync(gamma_sum_d, gamma_sum_zeros, nstates * sizeof(float), cudaMemcpyHostToDevice, stream), "failed to clear gamma_sum_d");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize gamma_sum_d clear");
	...
	for (int t = 0; t < length; t++) {
		...
		calc_gamma_dev<<<nblocks, threads_per_block, 0, stream>>>(gamma_sum_d, alpha_d, beta_d, nstates, t);
		cuda_check(cudaGetLastError(), "failed to launch calc_gamma_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize calc_gamma_dev");

		record_region_end(t);
	}
}

static int calc_xi_sum(cudaStream_t stream)
{
	float* xi_sum_zeros = (float*) calloc(nstates * nstates, sizeof(float));

	record_region_start("device_side_h2d_copy");

	cuda_check(cudaMemcpyAsync(xi_sum_d, xi_sum_zeros, nstates * nstates * sizeof(float), cudaMemcpyHostToDevice, stream), "failed to clear xi_sum_d");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize xi_sum_d clear");
	...
	for (int t = 0; t < length - 1; t++) {
		...
		calc_xi_dev<<<grid, threads, 0, stream>>>(xi_sum_d, a_d, b_d, alpha_d, beta_d, sum_ab, nstates, obs[t + 1], t);
		cuda_check(cudaGetLastError(), "failed to launch calc_xi_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize calc_xi_dev");
...
static int estimate_a(cudaStream_t stream)
{
	int nblocks = (nstates + BLOCK_DIM - 1) / BLOCK_DIM;
	dim3 grid(nblocks, nblocks);
	dim3 threads(BLOCK_DIM, BLOCK_DIM);
	...
	est_a_dev<<<grid, threads, 0, stream>>>(a_d, alpha_d, beta_d, xi_sum_d, gamma_sum_d, sum_ab, nstates, length);
	cuda_check(cudaGetLastError(), "failed to launch est_a_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize est_a_dev");
	...
	scale_a_dev<<<grid, threads, 0, stream>>>(a_d, c_d, nstates);
	cuda_check(cudaGetLastError(), "failed to launch scale_a_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize scale_a_dev");
...
static int estimate_b(cudaStream_t stream)
{
	float* b_d_zeros = (float*) calloc(nstates * nsymbols, sizeof(float));

	record_region_start("device_side_h2d_copy");

	cuda_check(cudaMemcpyAsync(b_d, b_d_zeros, nstates * nsymbols * sizeof(float), cudaMemcpyHostToDevice, stream), "failed to clear b_d");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize b_d clear");
	...
	for (int t = 0; t < length; t++) {
		...
		acc_b_dev<<<grid, threads, 0, stream>>>(b_d, alpha_d, beta_d, sum_ab, nstates, nsymbols, obs[t], t);
		cuda_check(cudaGetLastError(), "failed to launch acc_b_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize acc_b_dev");
	...
	est_b_dev<<<grid, threads, 0, stream>>>(b_d, gamma_sum_d, nstates, nsymbols);
	cuda_check(cudaGetLastError(), "failed to launch est_b_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize est_b_dev");
	...
	scale_b_dev<<<grid, threads, 0, stream>>>(b_d, c_d, nstates, nsymbols);
	cuda_check(cudaGetLastError(), "failed to launch scale_b_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize scale_b_dev");
...
static int estimate_pi(cudaStream_t stream)
{
	...
	est_pi_dev<<<nblocks, threads_per_block, 0, stream>>>(pi_d, alpha_d, beta_d, sum_ab, nstates);
	cuda_check(cudaGetLastError(), "failed to launch est_pi_dev");
	cuda_check(cudaStreamSynchronize(stream), "failed to synchronize est_pi_dev");
...
static float run_hmm_bwa(Hmm* hmm, Obs* in_obs, int iterations, float threshold, cudaStream_t stream)
{
	float* a = hmm->a;
	float* b = hmm->b;
	float* pi = hmm->pi;
	...
	cuda_check(cudaMalloc((void**) &a_d, sizeof(float) * nstates * nstates), "failed to allocate a_d");
	cuda_check(cudaMalloc((void**) &b_d, sizeof(float) * nstates * nsymbols), "failed to allocate b_d");
	cuda_check(cudaMalloc((void**) &pi_d, sizeof(float) * nstates), "failed to allocate pi_d");
	cuda_check(cudaMalloc((void**) &alpha_d, sizeof(float) * nstates * length), "failed to allocate alpha_d");
	cuda_check(cudaMalloc((void**) &beta_d, sizeof(float) * nstates * length), "failed to allocate beta_d");
	cuda_check(cudaMalloc((void**) &gamma_sum_d, sizeof(float) * nstates), "failed to allocate gamma_sum_d");
	cuda_check(cudaMalloc((void**) &xi_sum_d, sizeof(float) * nstates * nstates), "failed to allocate xi_sum_d");
	cuda_check(cudaMalloc((void**) &c_d, sizeof(float) * nstates), "failed to allocate c_d");
	cuda_check(cudaMalloc((void**) &ones_n_d, sizeof(float) * nstates), "failed to allocate ones_n_d");
	cuda_check(cudaMalloc((void**) &ones_s_d, sizeof(float) * nsymbols), "failed to allocate ones_s_d");
	...
	do {
		...
		cuda_check(cudaMemcpyAsync(a_d, a, sizeof(float) * nstates * nstates, cudaMemcpyHostToDevice, stream), "failed to copy a_d");
		cuda_check(cudaMemcpyAsync(b_d, b, sizeof(float) * nstates * nsymbols, cudaMemcpyHostToDevice, stream), "failed to copy b_d");
		cuda_check(cudaMemcpyAsync(pi_d, pi, sizeof(float) * nstates, cudaMemcpyHostToDevice, stream), "failed to copy pi_d");
		...
		init_ones_dev<<<nblocks, threads_per_block, 0, stream>>>(ones_s_d, nsymbols);
		cuda_check(cudaGetLastError(), "failed to launch init_ones_dev");
		cuda_check(cudaStreamSynchronize(stream), "failed to synchronize init_ones_dev");
...
int main(int argc, char* argv[])
{
	...
	cuda_check(cudaSetDevice(cuda_device), "failed to set CUDA device");
