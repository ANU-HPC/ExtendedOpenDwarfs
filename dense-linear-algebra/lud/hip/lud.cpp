...
int main(int argc, char** argv)
{
	...
	do {
		...
		hip_check(hipStreamSynchronize(stream), "failed to synchronize H2D copy");
...
#endif
			hip_check(
				lud_launch_diagonal_hip(d_m, matrix_dim, i, BLOCK_SIZE, stream),
				"failed to launch diagonal kernel");
			hip_check(hipStreamSynchronize(stream), "failed to synchronize diagonal kernel");
#ifndef PROFILE_OUTER_LOOP
			record_region_end(i);
#endif
...
#endif
			hip_check(
				lud_launch_perimeter_hip(d_m, matrix_dim, i, BLOCK_SIZE, blocks, stream),
				"failed to launch perimeter kernel");
			hip_check(hipStreamSynchronize(stream), "failed to synchronize perimeter kernel");
#ifndef PROFILE_OUTER_LOOP
			record_region_end(i);
#endif
...
#endif
			hip_check(
				lud_launch_internal_hip(d_m, matrix_dim, i, BLOCK_SIZE, blocks, stream),
				"failed to launch internal kernel");
			hip_check(hipStreamSynchronize(stream), "failed to synchronize internal kernel");
#ifndef PROFILE_OUTER_LOOP
			record_region_end(i);
#endif
		}
		...
		hip_check(hipStreamSynchronize(stream), "failed to synchronize final diagonal kernel");
	...
	hip_check(hipFree(d_m), "failed to free device matrix");
