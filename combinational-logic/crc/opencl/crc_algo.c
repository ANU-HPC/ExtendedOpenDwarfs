...
unsigned int page_size = DATA_SIZE, num_wg_sizes = 0, num_words;
unsigned int num_blocks, num_pages_last_block, num_block_sizes = 0;
	...
	cl_event* read_page)
{
	...
	LSB_Res();
	if (verbosity >= 2)
		printf("enqueueCRCDevice(): global_size=%zd - local_size=%zd\n", global_size, local_size);
...
int main(int argc, char** argv)
{
	...
	unsigned int run_serial = 0, h, ii, i, k, l;
	unsigned int num_pages = 1, num_execs = 1, num_kernels = 0;
	char* file = NULL;
	char* optptr;
	...
	CHKERR(err, "Failed to create write command queue!");

	read_queue = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
	CHKERR(err, "Failed to create read command queue!");
	...
	while ((c = getopt(argc, argv, "avn:s:i:p:w:k:hr:")) != -1)
	{
		switch (c)
		{
			...
			case 'p':
				...
				check(tmp != NULL, "crc.main() - Heap Overflow! Cannot allocate space for num_parallel_crcs");
			...
			case 'w':
				...
				check(tmp != NULL, "crc.main() - Heap Overflow! Cannot allocate space for wg_sizes");
				wg_sizes = tmp;
			...
			case 'k':
				...
				check(tmp != NULL, "crc.main() - Heap Overflow! Cannot allocate space for kernel_files");
				kernel_files = tmp;
	...
	{
		if (verbosity)
			printf("Executing with block size #%u of %u: %u\n", h + 1, num_block_sizes, num_parallel_crcs[h]);
		...
		if (verbosity)
			printf("Num Pages: %u - Num Parallel CRCs: %u - Num blocks = %u\n", num_pages, num_parallel_crcs[h], num_blocks);
		...
		for (i = 0; i < num_blocks; i++)
		{
			...
			CHKERR(err, "Failed to allocate device input memory!");

			dev_output[i] = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * num_parallel_crcs[h], NULL, &err);
			CHKERR(err, "Failed to allocate device output memory!");
		...
		{
			if (verbosity)
				printf("Executing with kernel #%u of %u: %s\n", l + 1, num_kernels, kernel_files[l]);
			...
			do {
				...
				{
					if (verbosity)
						printf("Executing with Workgroup size #%u of %u: %zu\n", k + 1, num_wg_sizes, wg_sizes[k]);

					for (ii = 0; ii < num_execs; ii++)
					{
						if (verbosity)
							printf("Beginning execution #%u of %u...\n", ii + 1, num_execs);
						...
						{
							if (verbosity >= 2)
								printf("\tEnqueuing commands for block #%u of %u...\n", i + 1, num_blocks);
							...
							if (verbosity >= 2)
								printf("\tmain(): global_size=%zd - local_size=%zd\n", global_size, local_size);
