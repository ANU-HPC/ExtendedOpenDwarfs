...
extern "C" hipError_t crc_upload_table_hip(const unsigned int table[8][256]);

extern "C" hipError_t crc_launch_hip(
...
void usage()
{
	...
	printf("\t-w : HIP threads per block, default 256\n");
	exit(0);
}
	...
	unsigned int block_id)
{
	...
	hip_check(hipStreamSynchronize(stream), "failed to synchronize CRC kernel");
	LSB_Rec(block_id);
...
int main(int argc, char** argv)
{
	...
	LSB_Res();
	hip_check(crc_upload_table_hip(crc32LookupHost), "failed to upload CRC lookup table");
	LSB_Rec(0);
					...
					if (run_serial)
					{
						...
						for (i = 0; i < num_pages; i++)
						{
							...
							if (cpu_remainder != ocl_remainders[i])
							{
								fprintf(
									stderr,
									"ERROR: HIP and CPU Slice-by-8 remainders for page %u differ [HIP: '%X', CPU: '%X']\n",
									i + 1,
									ocl_remainders[i],
									cpu_remainder);
							}
