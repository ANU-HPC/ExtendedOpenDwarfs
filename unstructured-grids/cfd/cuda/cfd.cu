...
static void upload(T* dst, const T* src, size_t n)
{
    ...
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(dst, src, sizeof(T) * n, cudaMemcpyHostToDevice));
...
static void download(T* dst, const T* src, size_t n)
{
    ...
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(dst, src, sizeof(T) * n, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaDeviceSynchronize());
    LSB_Rec(0);
}
...
int main(int argc, char** argv)
{
    ...
    CUDA_CHECK(cudaMemset(step_factors, 0, sizeof(float) * nelr));
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaFree(areas));
    CUDA_CHECK(cudaFree(elements_surrounding_elements));
    CUDA_CHECK(cudaFree(normals));
    CUDA_CHECK(cudaFree(ff_variable));
    CUDA_CHECK(cudaFree(ff_fc_momentum_x));
    CUDA_CHECK(cudaFree(ff_fc_momentum_y));
    CUDA_CHECK(cudaFree(ff_fc_momentum_z));
    CUDA_CHECK(cudaFree(ff_fc_density_energy));
    CUDA_CHECK(cudaFree(variables));
    CUDA_CHECK(cudaFree(old_variables));
    CUDA_CHECK(cudaFree(fluxes));
    CUDA_CHECK(cudaFree(step_factors));
    CUDA_CHECK(cudaFree(fc_momentum_x));
    CUDA_CHECK(cudaFree(fc_momentum_y));
    CUDA_CHECK(cudaFree(fc_momentum_z));
    CUDA_CHECK(cudaFree(fc_density_energy));
