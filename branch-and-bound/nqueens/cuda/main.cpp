...
// Adapted from OpenDwarfs OpenCL N-Queens
...
{
    std::cerr << "N-Queen solver for CUDA\n";
    std::cerr << "Ping-Che Chen / OpenDwarfs CUDA port\n\n";
    std::cerr << "Usage: " << argv0 << " [options] N\n";
    std::cerr << "\tN: board size (1 ~ 32)\n";
    std::cerr << "\t-cpu: use CPU\n";
    std::cerr << "\t-threads #: set number of CUDA threads/work items\n";
    std::cerr << "\t-blocksize #: set CUDA block size\n";
    std::cerr << "\t-local: accepted for OpenCL compatibility; ignored in CUDA scalar port\n";
    std::cerr << "\t-noatomics: accepted for compatibility; scalar CUDA port does not use atomics\n";
    std::cerr << "\t-novec: accepted for compatibility; scalar CUDA port is non-vectorized\n";
    std::cerr << "\t-vec4: accepted for compatibility; ignored in CUDA scalar port\n";
}
...
int main(int argc, char** argv)
{
    ...
    try {
        if (force_cpu) {
            ...
            NQueenSolverCUDA nqueen(threads,
                                    block_size,
                                    local,
                                    noatomics,
                                    novec,
