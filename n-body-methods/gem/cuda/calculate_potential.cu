    ...
    int step_size)
{
    ...
    CUDA_CHECK(cudaMalloc((void**)&res_c_d, sizeof(float) * nres));
    CUDA_CHECK(cudaMalloc((void**)&res_x_d, sizeof(float) * nres));
    CUDA_CHECK(cudaMalloc((void**)&res_y_d, sizeof(float) * nres));
    CUDA_CHECK(cudaMalloc((void**)&res_z_d, sizeof(float) * nres));

    CUDA_CHECK(cudaMalloc((void**)&at_c_d, sizeof(float) * natoms));
    CUDA_CHECK(cudaMalloc((void**)&at_x_d, sizeof(float) * natoms));
    CUDA_CHECK(cudaMalloc((void**)&at_y_d, sizeof(float) * natoms));
    CUDA_CHECK(cudaMalloc((void**)&at_z_d, sizeof(float) * natoms));

    CUDA_CHECK(cudaMalloc((void**)&vert_c_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_x_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_y_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_z_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_x_p_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_y_p_d, sizeof(float) * nvert));
    CUDA_CHECK(cudaMalloc((void**)&vert_z_p_d, sizeof(float) * nvert));

    CUDA_CHECK(cudaMalloc((void**)&atom_addrs_d, sizeof(unsigned int) * nres));
    CUDA_CHECK(cudaMalloc((void**)&atom_lengths_d, sizeof(unsigned int) * nres));
    ...
    do {
        ...
        CUDA_CHECK(cudaMemcpy(res_c_d, res_c, sizeof(float) * nres, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(res_x_d, res_x, sizeof(float) * nres, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(res_y_d, res_y, sizeof(float) * nres, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(res_z_d, res_z, sizeof(float) * nres, cudaMemcpyHostToDevice));

        CUDA_CHECK(cudaMemcpy(at_c_d, at_c, sizeof(float) * natoms, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(at_x_d, at_x, sizeof(float) * natoms, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(at_y_d, at_y, sizeof(float) * natoms, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(at_z_d, at_z, sizeof(float) * natoms, cudaMemcpyHostToDevice));

        CUDA_CHECK(cudaMemcpy(vert_c_d, vert_c, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_x_d, vert_x, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_y_d, vert_y, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_z_d, vert_z, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_x_p_d, vert_x_p, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_y_p_d, vert_y_p, sizeof(float) * nvert, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(vert_z_p_d, vert_z_p, sizeof(float) * nvert, cudaMemcpyHostToDevice));

        CUDA_CHECK(cudaMemcpy(atom_addrs_d, atom_addrs, sizeof(unsigned int) * nres, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(atom_lengths_d, atom_lengths, sizeof(unsigned int) * nres, cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(vert_c, vert_c_d, sizeof(float) * nvert, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaFree(res_c_d));
    CUDA_CHECK(cudaFree(res_x_d));
    CUDA_CHECK(cudaFree(res_y_d));
    CUDA_CHECK(cudaFree(res_z_d));
    CUDA_CHECK(cudaFree(at_c_d));
    CUDA_CHECK(cudaFree(at_x_d));
    CUDA_CHECK(cudaFree(at_y_d));
    CUDA_CHECK(cudaFree(at_z_d));
    CUDA_CHECK(cudaFree(vert_c_d));
    CUDA_CHECK(cudaFree(vert_x_d));
    CUDA_CHECK(cudaFree(vert_y_d));
    CUDA_CHECK(cudaFree(vert_z_d));
    CUDA_CHECK(cudaFree(vert_x_p_d));
    CUDA_CHECK(cudaFree(vert_y_p_d));
    CUDA_CHECK(cudaFree(vert_z_p_d));
    CUDA_CHECK(cudaFree(atom_addrs_d));
    CUDA_CHECK(cudaFree(atom_lengths_d));
