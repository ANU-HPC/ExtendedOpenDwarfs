...
static double checksum_component(const char* label, cl_mem component, int samplesNum)
{
    ...
    LSB_Set_Rparam_string("region", "device_side_d2h_copy");
    LSB_Res();
    errNum = clEnqueueReadBuffer(commandQueue, component, CL_TRUE, 0,
                                 (size_t)samplesNum * sizeof(int),
                                 host, 0, NULL, NULL);
    clFinish(commandQueue);
    LSB_Rec(0);
    CHKERR(errNum, "Failed to read DWT checksum buffer");
...
void processDWT(struct dwt *d, int forward, int writeVisual)
{
    ...
    if (d->components == 3) {
        ...
        checksum_component("r", cl_c_r_out, d->pixWidth * d->pixHeight);
        checksum_component("g", cl_c_g_out, d->pixWidth * d->pixHeight);
        checksum_component("b", cl_c_b_out, d->pixWidth * d->pixHeight);
...
bool savePGM(char* filename, int width, int height, int* data){
    ...
    bool ok = savePGM(filename,width,height,buff);
    free(buff);
    return ok;
}
