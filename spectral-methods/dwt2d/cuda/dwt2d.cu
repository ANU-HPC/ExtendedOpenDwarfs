...
static void checksum_component(const char* label, int* component, int samplesNum)
{
    ...
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(host, component, (size_t)samplesNum * sizeof(int), cudaMemcpyDeviceToHost));
...
static void copySrcToComponents(int* d_r, int* d_g, int* d_b, unsigned char* h_src, int width, int height)
{
    ...
    LSB_Res();
    CUDA_CHECK(cudaMalloc((void**)&d_src, (size_t)pixels * 3 * sizeof(unsigned char)));
    LSB_Rec(0);

    LSB_Set_Rparam_string("region", "device_side_h2d_copy");
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(d_src, h_src, (size_t)pixels * 3 * sizeof(unsigned char), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaDeviceSynchronize());
    LSB_Rec(0);
...
static void copySrcToComponent(int* d_c, unsigned char* h_src, int width, int height)
{
    ...
    LSB_Set_Rparam_string("region", "device_side_buffer_setup");
    LSB_Res();
    CUDA_CHECK(cudaMalloc((void**)&d_src, (size_t)pixels * sizeof(unsigned char)));
    LSB_Rec(0);

    LSB_Set_Rparam_string("region", "device_side_h2d_copy");
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(d_src, h_src, (size_t)pixels * sizeof(unsigned char), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaDeviceSynchronize());
    LSB_Rec(0);
...
static void memCopy(int* dest, int* src, size_t sx, size_t sy)
{
    ...
    LSB_Res();
    CUDA_CHECK(cudaMemcpy(dest, src, sx * sy * sizeof(int), cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaDeviceSynchronize());
    LSB_Rec(0);
}
...
static void processDWT(struct dwt *d, int forward, int writeVisual)
{
    ...
    LSB_Set_Rparam_string("region", "device_side_buffer_setup");
    LSB_Res();
    CUDA_CHECK(cudaMalloc((void**)&c_r_out, componentSize));
    CUDA_CHECK(cudaMalloc((void**)&backup, componentSize));

    if (d->components == 3) {
        CUDA_CHECK(cudaMalloc((void**)&c_g_out, componentSize));
        CUDA_CHECK(cudaMalloc((void**)&c_b_out, componentSize));
        CUDA_CHECK(cudaMalloc((void**)&c_r, componentSize));
        CUDA_CHECK(cudaMalloc((void**)&c_g, componentSize));
        CUDA_CHECK(cudaMalloc((void**)&c_b, componentSize));
    } else {
        CUDA_CHECK(cudaMalloc((void**)&c_r, componentSize));
    }
    LSB_Rec(0);
    ...
    if (d->components == 3) {
        ...
        CUDA_CHECK(cudaFree(c_g_out));
        CUDA_CHECK(cudaFree(c_b_out));
