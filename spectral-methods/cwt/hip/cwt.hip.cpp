...
static void check_hip(hipError_t err, const char *what)
{
    if (err != hipSuccess) {
        ...
        os << what << " failed: " << hipGetErrorString(err);
...
int main(int argc, char **argv)
{
    try {
        ...
        LSB_Res();
        check_hip(hipFree(0), "hip runtime initialization");
        hipDeviceProp_t prop{};
        int dev = 0;
        check_hip(hipGetDevice(&dev), "hipGetDevice");
        check_hip(hipGetDeviceProperties(&prop, dev), "hipGetDeviceProperties");
        LSB_Rec(0);
        ...
        check_hip(hipMalloc((void **)&d_fx, sizeof(real_t) * fx.size()), "hipMalloc fx");
        check_hip(hipMalloc((void **)&d_scales, sizeof(real_t) * scales.size()), "hipMalloc scales");
        check_hip(hipMalloc((void **)&d_trans, sizeof(real_t) * trans.size()), "hipMalloc trans");
        check_hip(hipMalloc((void **)&d_cwt, sizeof(real_t) * cwt.size()), "hipMalloc cwt");
        LSB_Rec(0);

        LSB_Set_Rparam_string("region", "device_side_h2d_copy");
        LSB_Res();
        check_hip(hipMemcpy(d_fx, fx.data(), sizeof(real_t) * fx.size(), hipMemcpyHostToDevice), "copy fx");
        check_hip(hipMemcpy(d_scales, scales.data(), sizeof(real_t) * scales.size(), hipMemcpyHostToDevice), "copy scales");
        check_hip(hipMemcpy(d_trans, trans.data(), sizeof(real_t) * trans.size(), hipMemcpyHostToDevice), "copy trans");
        check_hip(hipDeviceSynchronize(), "sync h2d");
        LSB_Rec(0);
        ...
        hipLaunchKernelGGL(
            ...
            kN,
            dt);
        check_hip(hipGetLastError(), "launch cwt_forward_kernel");
        check_hip(hipDeviceSynchronize(), "sync kernel");
        LSB_Rec(0);

        LSB_Set_Rparam_string("region", "device_side_d2h_copy");
        LSB_Res();
        check_hip(hipMemcpy(cwt.data(), d_cwt, sizeof(real_t) * cwt.size(), hipMemcpyDeviceToHost), "copy cwt");
        check_hip(hipDeviceSynchronize(), "sync d2h");
        LSB_Rec(0);

        LSB_Set_Rparam_string("region", "runtime_finalization");
        LSB_Res();
        check_hip(hipDeviceSynchronize(), "runtime final sync");
        LSB_Rec(0);
        ...
        std::cout << "CWT_HIP device=\"" << prop.name << "\""
                  << " N=" << N
                  << " B=" << B
                  << " A=" << A
                  << " values=" << cwt.size()
