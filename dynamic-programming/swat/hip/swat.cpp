...
extern "C" hipError_t swat_launch_match_hip(
	char* pathFlag,
	char* extFlag,
	float* nGapDist,
	float* hGapDist,
...
extern "C" hipError_t swat_launch_traceback_hip(
	char* pathFlag,
	char* extFlag,
	int* diffPos,
	char* seq1,
...
int main(int argc, char** argv)
{
	...
	hip_check(hipMalloc((void**) &seq1D, sizeof(char) * MAX_LEN), "failed to allocate seq1D");
	hip_check(hipMalloc((void**) &seq2D, sizeof(char) * MAX_LEN), "failed to allocate seq2D");
	hip_check(hipMalloc((void**) &outSeq1D, sizeof(char) * MAX_LEN * 2), "failed to allocate outSeq1D");
	hip_check(hipMalloc((void**) &outSeq2D, sizeof(char) * MAX_LEN * 2), "failed to allocate outSeq2D");
	hip_check(hipMalloc((void**) &threadNumD, sizeof(int) * 2 * MAX_LEN), "failed to allocate threadNumD");
	hip_check(hipMalloc((void**) &diffPosD, sizeof(int) * 2 * MAX_LEN), "failed to allocate diffPosD");
	hip_check(hipMalloc((void**) &pathFlagD, sizeof(char) * maxElemNum), "failed to allocate pathFlagD");
	hip_check(hipMalloc((void**) &extFlagD, sizeof(char) * maxElemNum), "failed to allocate extFlagD");
	hip_check(hipMalloc((void**) &nGapDistD, sizeof(float) * maxElemNum), "failed to allocate nGapDistD");
	hip_check(hipMalloc((void**) &hGapDistD, sizeof(float) * maxElemNum), "failed to allocate hGapDistD");
	hip_check(hipMalloc((void**) &vGapDistD, sizeof(float) * maxElemNum), "failed to allocate vGapDistD");
	hip_check(hipMalloc((void**) &maxInfoD, sizeof(MAX_INFO) * mfThreadNum), "failed to allocate maxInfoD");
	hip_check(hipMalloc((void**) &blosum62D, sizeof(float) * 23 * 23), "failed to allocate blosum62D");
	hip_check(hipMalloc((void**) &mutexMem, sizeof(int)), "failed to allocate mutexMem");
	...
	hip_check(hipStreamSynchronize(stream), "failed to synchronize BLOSUM copy");
	...
	do {
		...
		for (int subSequenceNo = 0; subSequenceNo < subSequenceNum; subSequenceNo++) {
			...
			hip_check(hipMemsetAsync(pathFlagD, 0, DPMatrixSize * sizeof(char), stream), "failed to clear pathFlagD");
			hip_check(hipMemsetAsync(extFlagD, 0, DPMatrixSize * sizeof(char), stream), "failed to clear extFlagD");
			hip_check(hipMemsetAsync(nGapDistD, 0, matrixIniNum * sizeof(float), stream), "failed to clear nGapDistD");
			hip_check(hipMemsetAsync(hGapDistD, 0, matrixIniNum * sizeof(float), stream), "failed to clear hGapDistD");
			hip_check(hipMemsetAsync(vGapDistD, 0, matrixIniNum * sizeof(float), stream), "failed to clear vGapDistD");
			hip_check(hipMemsetAsync(maxInfoD, 0, sizeof(MAX_INFO) * mfThreadNum, stream), "failed to clear maxInfoD");
			hip_check(hipMemsetAsync(mutexMem, 0, sizeof(int), stream), "failed to clear mutexMem");
			hip_check(hipStreamSynchronize(stream), "failed to synchronize buffer fill");

			record_region_end(subSequenceNo);

			record_region_start("device_side_h2d_copy");

			hip_check(hipMemcpyAsync(seq1D, seq1, (rowNum - 1) * sizeof(char), hipMemcpyHostToDevice, stream), "failed to copy seq1");
			hip_check(hipMemcpyAsync(seq2D, seq2, (columnNum - 1) * sizeof(char), hipMemcpyHostToDevice, stream), "failed to copy seq2");
			hip_check(hipMemcpyAsync(diffPosD, diffPos, launchNum * sizeof(int), hipMemcpyHostToDevice, stream), "failed to copy diffPos");
			hip_check(hipMemcpyAsync(threadNumD, threadNum, launchNum * sizeof(int), hipMemcpyHostToDevice, stream), "failed to copy threadNum");
			...
			hip_check(hipStreamSynchronize(stream), "failed to synchronize SWAT match kernel");
			...
			hip_check(hipStreamSynchronize(stream), "failed to synchronize SWAT traceback kernel");
			...
			hip_check(hipMemcpyAsync(maxInfo, maxInfoD, sizeof(MAX_INFO), hipMemcpyDeviceToHost, stream), "failed to copy maxInfo");

			int maxOutputLen = rowNum + columnNum - 2;

			hip_check(hipMemcpyAsync(outSeq1, outSeq1D, maxOutputLen * sizeof(char), hipMemcpyDeviceToHost, stream), "failed to copy outSeq1");
			hip_check(hipMemcpyAsync(outSeq2, outSeq2D, maxOutputLen * sizeof(char), hipMemcpyDeviceToHost, stream), "failed to copy outSeq2");

			hip_check(hipStreamSynchronize(stream), "failed to synchronize D2H copies");
	...
	hip_check(hipFree(seq1D), "failed to free seq1D");
	hip_check(hipFree(seq2D), "failed to free seq2D");
	hip_check(hipFree(outSeq1D), "failed to free outSeq1D");
	hip_check(hipFree(outSeq2D), "failed to free outSeq2D");
	hip_check(hipFree(threadNumD), "failed to free threadNumD");
	hip_check(hipFree(diffPosD), "failed to free diffPosD");
	hip_check(hipFree(pathFlagD), "failed to free pathFlagD");
	hip_check(hipFree(extFlagD), "failed to free extFlagD");
	hip_check(hipFree(nGapDistD), "failed to free nGapDistD");
	hip_check(hipFree(hGapDistD), "failed to free hGapDistD");
	hip_check(hipFree(vGapDistD), "failed to free vGapDistD");
	hip_check(hipFree(maxInfoD), "failed to free maxInfoD");
	hip_check(hipFree(blosum62D), "failed to free blosum62D");
	hip_check(hipFree(mutexMem), "failed to free mutexMem");
