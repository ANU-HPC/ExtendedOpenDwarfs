...
extern "C" cudaError_t needle_launch_cuda_shared_1(
	const int* reference,
	int* matrix,
	int cols,
	int penalty,
...
extern "C" cudaError_t needle_launch_cuda_shared_2(
	const int* reference,
	int* matrix,
	int cols,
	int penalty,
...
int main(int argc, char** argv)
{
	...
	cuda_check(
		cudaMalloc((void**) &d_referrence, sizeof(int) * size),
		"failed to allocate reference buffer");

	cuda_check(
		cudaMalloc((void**) &d_matrix, sizeof(int) * size),
	...
	do {
		...
		for (int i = 1; i <= block_width; i++) {
			...
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize NW kernel 1");
		...
		for (int i = block_width - 1; i >= 1; i--) {
			...
			cuda_check(cudaStreamSynchronize(stream), "failed to synchronize NW kernel 2");
	...
	cuda_check(cudaFree(d_referrence), "failed to free reference buffer");
	cuda_check(cudaFree(d_matrix), "failed to free matrix buffer");
