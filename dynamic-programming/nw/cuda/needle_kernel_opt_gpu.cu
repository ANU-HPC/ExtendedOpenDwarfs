...
int maximum_opt(int a, int b, int c)
{
	int k = (a <= b) ? b : a;
	return (k <= c) ? c : k;
}
...
extern "C" cudaError_t needle_launch_cuda_shared_1(
	...
	cudaStream_t stream)
{
	needle_cuda_shared_opt_1<<<dim3(i), dim3(BLOCK_SIZE), 0, stream>>>(
		referrence,
		matrix_cuda,
		cols,
		penalty,
...
extern "C" cudaError_t needle_launch_cuda_shared_2(
	...
	cudaStream_t stream)
{
	needle_cuda_shared_opt_2<<<dim3(i), dim3(BLOCK_SIZE), 0, stream>>>(
		referrence,
		matrix_cuda,
		cols,
		penalty,
